...
def main():
    with tf.Session() as sess:
        ...
        for col in range(1,7):
            sheet.cell(row = excel_row,column = col+12).value = Y[col-1][0]
        ...
        for col in range(1,7):
            sheet.cell(row = excel_row,column = col+18).value = P[col-1][col-1]
        for col in range(1,7):
            sheet.cell(row = excel_row,column = col+6).value = X[col-1][0]
        for col in range(1,7):
            sheet.cell(row = excel_row,column = col).value = X_pred[col-1][0]
        excel_row+=1
        count = 0
        global client
        try:
            ...
            while(True):
                ...
                pp_x, pp_y, pp_depth, capture = IntelRealsense.pingpong_detection(display = True)   # Positional Detection
                pingpong_camera = IntelRealsense.reverse_perspective_projection(pp_x, pp_y, pp_depth); pingpong_camera[3] = 1   # Real World Positional Projection
                processing = capture - last_capture; timestep = processing  # Define timestep values
                # print('Distance from camera \n%s' %(pingpong_camera))
                pingpong_base = T_camera2base.dot(pingpong_camera)  # changing reference frame to base frame
                # print('Distance from base UR5 %s (%s)' %(pingpong_base, timestep))
                v_x, v_depth, v_z, a_x, a_y, a_depth, STATE = IntelRealsense.pingpong_velocity(STATE, pingpong_base[0,0], pingpong_base[1,0], pingpong_base[2,0], capture, display = False)     # Velocity Detection
                ...
                if STATE != 'NONE':
                    ...
                    if STATE == 'VELOCITY':
                        X, P, X_pred = init_detection_state(X, P, Y, timestep)
                        # print('P: %s' %P)
                        for col in range(1,7):
                            sheet.cell(row = excel_row,column = col+18).value = P[col-1][col-1]
                        for col in range(1,7):
                        ...
                        excel_row+=1
                        # print('X %s' %(X))
                        # sphere_eq = X[0,0]**2 + X[1,0]**2 + X[2,0]**2
                        # print('sphere_eq %s' %(sphere_eq))
                        count += 1;
                        if count == 20:book.save('/home/s/catkin_ws/src/ur_modern_driver/Kalman_Filter.xlsx');print('xlsx saved...')
                    # if STATE == 'GRASPING':
                        # pingpong_camera[0:3] = X[0:3]
